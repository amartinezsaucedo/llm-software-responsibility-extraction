...
class LlamaBuilder(LLMBuilder):
    ...
    def set_system_message(self, system_message: str = None):
        if system_message:
            ...
            You list from the following requirement the responsibilities. To identify responsibilities use the following rules:
            Rules: 
    ...
    def set_memory(self):
        ...
        self._memory.save_context({"input": "If a student accesses the system then the student can add a new course or drop an added course"}, {"output": "access system, add course, drop course"})
        self._memory.save_context({"input": "After the student adds a course, the system sends the transaction information to the billing system"}, {"output": "add course, send information to billing system"})

    def get_llm(self) -> LLM:
        callback_manager = CallbackManager([StreamingStdOutCallbackHandler()])
        self._llm = LlamaCpp(callback_manager=callback_manager, **self._model_configuration,
                             **self._inference_configuration)
        return LLM(self._llm, self._prompt, self._memory, self._output_parser)